...
#define ValveSelect1 13
#define ValveSelect2 6
...
void lcdBooting()
{
  ...
  int titleLen = strlen(getText(TXT_APP_TITLE));
  lcd.setCursor((16 - titleLen) / 2, 0); 
  lcd.print(getText(TXT_APP_TITLE));


  int bootingLen = strlen(getText(TXT_BOOTING));
  lcd.setCursor((16 - bootingLen) / 2, 1);
...
void setup() {
  ...
  pinMode(ValveSelect1, OUTPUT);
  pinMode(ValveSelect2, OUTPUT);

  pinMode(HBS1C, OUTPUT);
  pinMode(HBS2C, OUTPUT);
  pinMode(HBS3C, OUTPUT);
  pinMode(HBS4C, OUTPUT);

  pinMode(CGS,OUTPUT);
  pinMode(VDS,OUTPUT);
  pinMode(AMS,OUTPUT);

// EZT ki kell majd venni

  digitalWrite(HBS1C, LOW);
  digitalWrite(HBS2C, LOW);
  digitalWrite(HBS3C, LOW);
  digitalWrite(HBS4C, LOW);

  digitalWrite(CGS, LOW);
  digitalWrite(VDS, LOW);
...
void setValve1()
{
  digitalWrite(ValveSelect1, HIGH);
  digitalWrite(ValveSelect2, HIGH);
}

void setValve2()
{
  digitalWrite(ValveSelect1, LOW);
  digitalWrite(ValveSelect2, LOW);
}

void connectCG()
{
  digitalWrite(CGS, HIGH);
}
...
void printCentered(float value, int precision, int row) {
  char buffer[17]; 
  dtostrf(value, 1, precision, buffer); 
  printCentered(buffer, row); 
}
...
float getAveragedVoltage(float multipler) {
  int32_t sum = 0;
  ...
  float averageRaw = (float)sum / numReadings;

  float voltage = (averageRaw * multipler) / 1000.0;
...
float calculateVDResistance(float measuredVoltage) {
  float U_be = 5.0;  
  float R1 = 1000.0; 

  float resistance = R1 * (measuredVoltage / (U_be - measuredVoltage));

  return resistance;
}

float calculateCGResistance(float measuredVoltage) {

  const float contsCurrent = 0.056818;
  float resistance = measuredVoltage / contsCurrent;
...
{
  lcd.clear();
  printCentered(getText(TXT_MENU_FULL_OUT_TEST), 0);
  
  // Alapértelmezett kezdőállapot
  printCentered("KI", 1);
  digitalWrite(HBS2C, LOW);
  digitalWrite(HBS3C, LOW);

  while(digitalRead(StartButton) == 0) { delay(10); }
  delay(50); 

  bool exitTest = false; 
  
  uint8_t localLastLeft = 1;
  uint8_t localLastRight = 1;

  while(!exitTest)
  {

    uint8_t currentLeft = digitalRead(GoLeftButton);
    if (currentLeft == 0 && localLastLeft == 1)
    {
      printCentered("KI", 1);
      digitalWrite(HBS2C, LOW);
      digitalWrite(HBS3C, LOW);
    ...
    localLastLeft = currentLeft;


    uint8_t currentRight = digitalRead(GoRightButton);
    if (currentRight == 0 && localLastRight == 1)
    {
      printCentered("BE", 1);
      digitalWrite(HBS2C, HIGH);
      digitalWrite(HBS3C, HIGH);
      
      delay(50);
    }
    localLastRight = currentRight;
  ...
  printCentered("Teszt Vege", 1);
  printCentered("Kilepes", 2);
  delay(1000);
  digitalWrite(HBS2C, LOW);
  digitalWrite(HBS3C, LOW);
...
void ContTest()
{
  if (ValveNumber == 1)
  {
    ...
    float voltage = getAveragedVoltage(0.015625);
    
    if (voltage != -1)
    {
      printCentered("Folytonos",1);
      printCentered("<100 Ohm",2);
    }

    else
    {
      printCentered("Szakadt",1);
      printCentered(">100 Ohm",2);
    }
...
void OhmTest()
{
  if (ValveNumber == 1)
  {
    ...
    if (voltage != -1)
    {
      float resisntace = calculateVDResistance(voltage);

      if (resisntace > 29)
      {
      lcd.setCursor(0,2);
      printCentered((String(resisntace,2) + " Ohm").c_str(), 2);
      }
      else
      {
        disconnectVD();
        ...
        voltage = getAveragedVoltage(0.0625);
        resisntace = calculateCGResistance(voltage);
        ...
        else
        {
          ...
          voltage = getAveragedVoltage(0.03125);
          resisntace = calculateCGResistance(voltage);
          ...
          {
            ads.setGain(GAIN_EIGHT);
            voltage = getAveragedVoltage(0.015625);
            resisntace = calculateCGResistance(voltage);
            lcd.setCursor(0,2);
            printCentered((String(resisntace,2) + " Ohm").c_str(), 2);
          }
